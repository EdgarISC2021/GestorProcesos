  ...
  return (
    <>
                      ...
                      <Box sx={{ display: 'flex', gap: 1, justifyContent: 'center' }}>
                        ...
                        <Tooltip title="Editar">
                          <IconButton 
                            size="small"
                            color="secondary"
                            onClick={() => navigate(`/solicitudes/editar/${solicitud.id_solicitud}`)}
                          >
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="md" fullWidth>
        <DialogTitle>
          Detalles de la Solicitud {selectedSolicitud?.folio}
        </DialogTitle>
        <DialogContent>
          {selectedSolicitud && (
            <>
              <Box sx={{ display: 'flex', mb: 3 }}>
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography>{selectedSolicitud.descripcion || '-'}</Typography>
                </Box>
                <Box sx={{ flex: 1 }}>
                  <Typography variant="subtitle2" color="text.secondary">Tipo de Área</Typography>
                  <Typography>{selectedSolicitud.tipo_area || '-'}</Typography>
                </Box>
              </Box>
              
              <Box sx={{ display: 'flex', mb: 3 }}>
                <Box sx={{ flex: 1 }}>
                  <Typography variant="subtitle2" color="text.secondary">Fecha Creación</Typography>
                  <Typography>{formatDate(selectedSolicitud.fecha_creacion)}</Typography>
                </Box>
                <Box sx={{ flex: 1 }}>
                  <Typography variant="subtitle2" color="text.secondary">Fecha Estimación</Typography>
                  <Typography>{formatDate(selectedSolicitud.fecha_estimacion)}</Typography>
                </Box>
              </Box>
              
              <Box sx={{ mb: 3 }}>
                <Typography variant="subtitle2" color="text.secondary">Responsable</Typography>
                <Typography>{selectedSolicitud.responsable_seguimiento || '-'}</Typography>
              </Box>
              
              <Box sx={{ mb: 3 }}>
                <Typography variant="subtitle2" color="text.secondary">Comentarios</Typography>
                <Typography>{selectedSolicitud.comentarios || 'Sin comentarios'}</Typography>
              </Box>
              
              <Box sx={{ mb: 3 }}>
                <Typography variant="subtitle2" color="text.secondary">Estado Actual</Typography>
                <Chip
                  label={selectedSolicitud.estatus || 'Pendiente'}
                  color={
                    selectedSolicitud.estatus === 'Aprobado' ? 'success' :
                    selectedSolicitud.estatus === 'Rechazado' ? 'error' :
                    selectedSolicitud.estatus === 'En revisión' ? 'info' : 'warning'
                  }
                  sx={{ mb: 2 }}
                />
              </Box>
              ...
              <Typography variant="subtitle1" gutterBottom>Cambiar Estado</Typography>
